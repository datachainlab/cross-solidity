...
contract MockCrossContractTest is Test {
    ...
    function test_onContractCommitImmediately_ReturnsExpectedBytes() public {
        CrossContext memory ctx = _mkContextSingle(bytes("tester"), AuthType.AuthMode.AUTH_MODE_CHANNEL);
        bytes memory callInfo = hex"01";

        bytes memory ret = mock.onContractCommitImmediately(ctx, callInfo);

        assertEq(ret, bytes("mock call succeed"));
    }

    function test_onContractCommitImmediately_RevertWhen_SignersLenIsZero() public {
        ...
        mock.onContractCommitImmediately(ctx, hex"01");
    }

    function test_onContractCommitImmediately_RevertWhen_SignersLenIsTwo() public {
    ...
    function test_onContractCommitImmediately_RevertWhen_AuthModeIsNotChannel() public {
    ...
    function test_onContractCommitImmediately_RevertWhen_UnexpectedAccountId() public {
    ...
    function test_onContractCommitImmediately_RevertWhen_CallInfoLenIsZero() public {
        CrossContext memory ctx = _mkContextSingle(bytes("tester"), AuthType.AuthMode.AUTH_MODE_CHANNEL);

        vm.expectRevert(bytes("the length of callInfo must be 1"));
        mock.onContractCommitImmediately(ctx, bytes(""));
    }

    function test_onContractCommitImmediately_RevertWhen_CallInfoLenIsTwo() public {
        CrossContext memory ctx = _mkContextSingle(bytes("tester"), AuthType.AuthMode.AUTH_MODE_CHANNEL);
        ...
        vm.expectRevert(bytes("the length of callInfo must be 1"));
        mock.onContractCommitImmediately(ctx, callInfo);
    }

    function test_onContractCommitImmediately_RevertWhen_CallInfoIsNot0x01() public {
        CrossContext memory ctx = _mkContextSingle(bytes("tester"), AuthType.AuthMode.AUTH_MODE_CHANNEL);
