...
library ChannelInfo {
     ...
     * @return The number of bytes decoded
     */
    function _read_port(uint256 p, bytes memory bs, Data memory r) internal pure returns (uint256) {
     ...
     * @return The number of bytes decoded
     */
    function _read_channel(uint256 p, bytes memory bs, Data memory r) internal pure returns (uint256) {
        (string memory x, uint256 sz) = ProtoBufRuntime._decode_string(p, bs);
        r.channel = x;
        return sz;
    }
    ...
    function _encode(Data memory r, uint256 p, bytes memory bs) internal pure returns (uint256) {
        ...
        if (bytes(r.channel).length != 0) {
            pointer += ProtoBufRuntime._encode_key(2, ProtoBufRuntime.WireType.LengthDelim, pointer, bs);
            pointer += ProtoBufRuntime._encode_string(r.channel, pointer, bs);
        }
    ...
    function _estimate(Data memory r) internal pure returns (uint256) {
        ...
        e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.channel).length);
        return e;
    }
    // empty checker
